			...
			<div class="main" id="viewBoard">
				<form id="frm">
					<input type="hidden" name="no" id="no" value='${board.no}' /> <input
						type="hidden" name="query" id="query" />
					<table border="1" width='400'>
						...
						<tbody>
							...
							<tr>
								<td colspan="4" valign="top" height="200"><pre>${board.contents}</pre>
								</td>
							</tr>
						</tbody>
						<tfoot>
							<tr>
								<td colspan="4" align="center"><a href="#"
									onclick="listBoard()">목록</a> <c:if test="${id==board.id}">
...
<script>
	// @@ updateBoardForm() 호출 시 form의 action을 updateBoardForm.do로 
	//    변경 후 submit 한다. 
	...
	// @@ deleteBoard() 호출 시 form의 action을 deleteBoard.do로 변경 후 
	//    submit 한다.
	...
	// @@ listBoard() 호출 시 request의 queryString을 이용해서 listBoard.do를 
	//    호출한다.
	function listBoard(){
