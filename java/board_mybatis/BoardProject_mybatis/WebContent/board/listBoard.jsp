...
<script>
	...
	<c:if test="${!empty param.key}">
		$("#key").val("${param.key}");	// select는 고정된 값 만!! 써야한다. : 공백 금지
	</c:if>
	...
	function find(){
		...
		$("#pagination-container").pagination({
			...
			locator: "list",						// JSON에서 데이터의 키값		
			totalNumberLocator:function(res){		// 전체 데이터의 개수 -->JSON에서 추출
				return res.totalNumber;
			},
			pageSize: 5,							// 한 화면에 보여줄 데이터 개수
