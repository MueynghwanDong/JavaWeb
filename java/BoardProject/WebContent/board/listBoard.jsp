				...
				<form id="frm">
					...
					<table border="1">
						...
						<tr align="center">
							...
							<input type="button" value="검색" id="search"> <c:if
									test="${not empty id }">
...
<!-- 	// @@ 전달받은 파라미터(key, word, pageNumber)로 검색 조건을 초기화 한다.  초기에는 값이 없고 검색 후, 페이지네이션 후 값들이 생성-->

		<c:if test="${!empty param.key}">
			$("#key").val("${param.key}");// select는 고정된 값 만!! 써야한다
		</c:if>
		...
		function find(){
			...
			$("#pagination-container").pagination({
				dataSource:"listBoardData.do?" + queryStr, // ajax 호출 경로 -> JSON 리턴
				locator : "list", //Json에서 데이터 값 읽기
				totalNumberLocator : function(res){ // 전체 데이터 갯수 -> JSON에서 추출
					//console.log(res);
					return res.totalNumber;		
				},
				pageSize : 5,		// 한 화면에 보여줄 데이터 갯수
				callback : function(data, pagination){ // 화면에 보여줄 데이터가 전달됨
